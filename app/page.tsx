...
export default function Home() {
  ...
  useEffect(() => {
    const savedReferrals = localStorage.getItem("giftmatch_referrals");
    const savedClaims = localStorage.getItem("giftmatch_claimed_rewards");
    const savedId = localStorage.getItem("giftmatch_referral_id");

    if (savedReferrals) {
      setReferrals(Number(savedReferrals));
    }
    ...
    if (savedId) {
      ...
      localStorage.setItem("giftmatch_referral_id", newId);
      setReferralId(newId);
    }
  }, []);

  useEffect(() => {
    localStorage.setItem("giftmatch_referrals", String(referrals));
  }, [referrals]);

  useEffect(() => {
    localStorage.setItem(
      "giftmatch_claimed_rewards",
 ...
 const referralLink = `https://giftmatch-taupe.vercel.app/?ref=${referralId}`;
  ...
  function claimReward(target: number) {
    if (referrals < target || claimedRewards.includes(target)) return;

    setClaimedRewards((current) => [...current, target]);
  }
    ...
    >
      ...
      >
        <div className="mx-auto max-w-7xl">
          <div className="mx-auto max-w-3xl text-center">
            ...
            >
              Choose who the mystery gift is for and discover surprise
      ...
      >
        <div className="mx-auto max-w-6xl">
          ...
          <div
            className={`mt-8 rounded-2xl border border-dashed p-5 text-center text-xs ${
              isHalloween
                ? "border-purple-800 text-purple-400"
                : "border-slate-300 text-slate-400"
            }`}
          >
            Demo mode: referral progress is stored in this browser. Real
            referral verification should be connected to a backend/database
            before offering real rewards.
          </div>
        </div>
      </section>
